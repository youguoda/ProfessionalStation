...
- **阶段 3（进行中）**：
  - ✅ 自动化规则引擎：超期自动标记青蛙 / 完成自动清除青蛙 / 等待超 7 天提醒（可开关 + 运行日志）。
  - ✅ 习惯追踪：Habit/HabitCheck 实体、7 日打卡网格（Atomic Habits）。
  - ✅ 智能辅助（本地启发式）：多行批量捕获、备注拆分为子任务、启发式智能排期建议（优先级 × 努力值 × 空闲槽位）。
  - ✅ 真实 LLM 接入：OpenAI 兼容接口（默认 DeepSeek，`AI_API_KEY`/`AI_BASE_URL`/`AI_MODEL` 配置），AI 拆分与 AI 排期；未配置或调用失败时自动降级到本地启发式，API Key 仅存在于服务端。
  - ✅ 「马力」AI 计划助手：可定制人格（4 套预设模板 + 角色/语气/风格/边界自由编辑），基于用户计划数据对话（今日/下一步/超期/等待/习惯自动注入），操作以建议卡片呈现、经确认后走状态机引擎执行（HITL）；对话历史与记忆笔记持久化。
  - ✅ 阶段 C：马力回复流式输出（SSE 打字机：上游 stream 转发 + 回复/建议两段式调用）；对话摘要滚动窗口（超 16 条把旧消息压缩进早期摘要，保留最近 8 条）；记忆笔记自动提炼（每轮对话后提炼用户偏好事实、去重入库、检索注入）。
  - ⏳ 多端同步 / 团队协作：留待后续。
